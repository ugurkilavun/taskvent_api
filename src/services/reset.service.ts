...
import { insertReset, findReset, updateReset } from "../repositories/reset.repository";
// Types
...
export const forgotPassword = async (email?: string): Promise<authResponseType> => {
  ...
  const DATA: any = await findByEmail(email);
  ...
  const hashedToken = hashURLToken(urlToken);

  const resetIn = await insertReset({
  ...
  sendPasswordResetLink({
    ...
    resetUrl: `${process.env.URL}/resetPassword?token=${urlToken}`,
    lang: DATA.country
  });
...
export const resetPassword = async (token?: string, password?: string, rePassword?: string): Promise<authResponseType> => {
  ...
  const TOKEN_: any = await findReset(token);
  ...
  const passwordBeenUpdated: any = await updatePasswordWithId(TOKEN_.id, paswdHash);
  if (!passwordBeenUpdated) throw new statusCodeErrors("Update error.", 400);

  const tokenBeenUpdated = await updateReset(TOKEN_.id, TOKEN_.token);
